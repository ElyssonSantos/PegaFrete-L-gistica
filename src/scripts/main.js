...
import { logSecurityEvent as logSecurityEventToDB, getSystemConfig, addSystemMessage, subscribeToSystemMessages, markSystemMessagesAsRead } from '../services/systemService.js';
...
import { initFCM, requestNotificationPermission, removeFCMToken } from '../core/fcm.js';
        ...
        if (user) {
            try {
                ...
                if (user.email) {
                    ...
                    registerPublicEmail(emailKey, providerId).catch(err => console.warn("Erro ao salvar public_emails:", err));
                }
                ...
                window.userDocListener = subscribeToUserProfile(user.uid, (data) => {
                    if (data) {
                        userData = data;
...
async function verificarEmailDisponivel(email) {
    ...
    try {
        const data = await getPublicEmailData(emailKey);
        if (data) {
...
async function finalizarCadastroGoogle() {
    ...
    await setUserProfile(uid, cleanUserData);
    userData = cleanUserData;

    // 2. Salva em public_emails para controle de login manual
    const emailKey = cleanUserData.email.toLowerCase().trim();
    await registerPublicEmail(emailKey, 'google').catch(err => console.warn("Erro ao salvar public_emails:", err));
...
function startFreightListener() {
    ...
    freightListenerUnsubscribe = subscribeToFreights((novosDados) => {
...
async function salvarEdicaoFrete(btn) {
    ...
    try {
        ...
        if (typeof currentEditId === 'string') {
            ...
            await updateFreight(currentEditId, updatedData);
        }
...
async function verificarTelefone(btn) {
    ...
    try {
        confirmationResult = await sendSMSVerification(phoneInput, 'recaptcha-container');
...
async function confirmarCodigo(btn) {
    ...
    try {
        ...
        const hasPhone = await checkPhoneExists(tempPhone);
...
async function fazerLogin(btn) {
    ...
    try {
        const authData = await loginWithEmail(email, pass);
        btn.innerHTML = original;
        ...
        if (error.code === 'auth/user-not-found' || error.code === 'auth/invalid-credential' || error.code === 'custom/user-not-found') {
    ...
    try {
        ...
        if (cleanUserData.role === 'driver') {
            try {
                ...
                if (sysConfig && sysConfig.autoApproveDrivers === true) {
        ...
        const emailKey = cleanUserData.email.toLowerCase().trim();
        await registerPublicEmail(emailKey, 'password').catch(err => console.warn("Erro ao salvar public_emails:", err));
...
async function salvarNovoEmail() {
    ...
    try {
        ...
        await deletePublicEmail(oldEmailKey).catch(() => console.warn("old email public_emails delete err"));
        await registerPublicEmail(newEmailKey, 'password').catch(() => console.warn("new email public_emails err"));
...
async function confirmDocUpload() {
    ...
    try {
        const uid = auth.currentUser.uid;
        let updates = { docStatus: 'Pendente' };
...
async function aceitarFrete(btn) {
    ...
    try {
        await acceptFreight(freight.id, auth.currentUser.uid);
...
function startSystemMessagesListener() {
    ...
    systemMessagesListener = subscribeToSystemMessages(uid, (messages) => {
...
window.concluirFrete = function (id) {
    ...
    if (!modal || !btn) {
        ...
        updateFreightStatus(id, 'entregue')
    ...
    btn.onclick = async () => {
        try {
            await updateFreightStatus(id, 'entregue');
...
window.cancelarFrete = function (id) {
    ...
    const btn = document.getElementById('confirmActionBtn');
    if (!modal || !btn) {
        updateFreightStatus(id, 'cancelado')
    ...
    btn.onclick = async () => {
        try {
            await updateFreightStatus(id, 'cancelado');
